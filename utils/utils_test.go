...
func mockTransactions() []types.Transaction {
...
func TestNewTransactionGroup(t *testing.T) {
	...
	assert.Equal(t, 2, len(txnGroup.SignedTransactions))
...
func TestSignWithLogicsig(t *testing.T) {
	...
	_, err := NewTransactionGroup(transactions)
	assert.Nil(t, err)

	logicsig := types.LogicSig{
	...
	add := crypto.AddressFromProgram(logicsig.Logic)

	assert.Equal(t, ed25519.PublicKeySize, len(add))
	fmt.Println(add)
	// assert.Equal(t, ed25519.PublicKeySize, n)
